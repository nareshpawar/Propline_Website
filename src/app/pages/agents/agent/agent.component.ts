...
export class AgentComponent implements OnInit {
  ...
  getAgentsList(){
    ...
    this.appService.getAgentsList().subscribe(res=>{
        this.agentData = res['data']?.map(ele=>{
          return   {
            id: ele.agent_id,
        ...
        this.agent = this.agentData.filter(agent=> agent.id == this.agentId)[0];
  ...
  public onContactFormSubmit(values:Object){
    if (this.contactForm.valid) { 
      this.appService.postContactME(values).subscribe(res=>{
        // console.log(res);
