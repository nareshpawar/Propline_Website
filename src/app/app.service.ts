...
export class AppService {
  ...
  // public baseUrl = 'http://appserver.preownedproperties.in:8080/pop-dev/api/';
  private ourtestimonialListApi = this.baseUrl + '/testimonial';
  private agentListApi = this.baseUrl + '/agents';

  rentalSubject = new Subject<any>();
  ...
  public getPropertyById(id): Observable<any> {
    ...
    return this.http.get<any>(this.baseUrl + `/property/byPropertyId?property_id=`+id);
  }

  getPropertyTypeController():Observable<any>{
    return this.http.get<any>(this.baseUrl + `/propertyType`);
  }
 
  geSocietyDetails(){
    return this.http.get<any>(this.baseUrl + `/society`);

  }

  getRentalSociety(){
    return this.http.get<any>(this.baseUrl + `/society/filterRent`);
  ...
  getactivity(id){
    ...
    return this.http.get<any>(this.baseUrl + `/owner/getActivity`,{headers});

  }

  postFeedback(feedbackObj){
    return this.http.post(this.baseUrl + '/feedback',feedbackObj)

  }

  postContactME(formData){
    let headers ={
      'agentEmail' : formData.email,
    }
    return this.http.post(this.baseUrl + '/agents/sendEmailToAgent',formData,{headers})
  ...
  contactUs(formData){
    return this.http.post(this.baseUrl +'/users/emailContact',formData);
  }
  ...
  public getHomeCarouselSlides() {
    ...
    return this.http.get<any>(this.baseUrl + `/master/banner`);
  }
  getCardDetails(id){
    const headers:any = { "property_id": String(id) };
    return this.http.get<any>(this.baseUrl + `/master/cards`,{headers});

  }

  filterByRental(data, propertyName?, sort?, page?, perPage?){    
      if (propertyName) {
        data = data.filter(property => property.propertyStatus[0] === "For Rent" && property.propertyType == propertyName )
      }else{
        data = data.filter(property => property.propertyStatus[0] === "For Rent");
      }
      this.sortData(sort, data);
      return this.paginator(data, page, perPage)
  }

  public filterData(data, params: any, sort?, page?, perPage?, propertyStatus?,rental?) {
    if (params) {
      if(params.bhk){
        data = data.filter(property => property.bedrooms === params.bhk);
      }
      if(params.location){
        data = data.filter(property => property.neighborhood[0].toLowerCase() === params.location.toLowerCase());
      }
      if(params.furnished){
        data = data.filter(property => property.furnishedStatus !=='' ? property.furnishedStatus == params.furnished :
                                       property.commercial_details.furnishedStatus == params.furnished);
      }
      if (params.propertyType && propertyStatus) {
        data = data.filter(property => property.propertyType == params.propertyType.property_name && property.propertyStatus[0] === propertyStatus)
      }
      if (propertyStatus && rental) {
        data = data.filter(property => property.propertyStatus[0] === propertyStatus);
      }
      if (params.propertyType) {
        data = data.filter(property => property.propertyType == params.propertyType.property_name)
      }
      if (params.society) {
        data = data.filter(property => property.society.society_id === params.society.society_id )
      }
      ...
      if (params.city) {
        if(propertyStatus){
          data = data.filter(property => property.city == params.city.name && property.propertyStatus[0] === propertyStatus)
        }else{
      ...
      if (params.bedrooms) {
        if(propertyStatus){
          if (params.bedrooms.from) {
            data = data.filter(property => property.bedrooms >= params.bedrooms.from && property.propertyStatus[0] === propertyStatus)
          }
          if (params.bedrooms.to) {
            data = data.filter(property => property.bedrooms <= params.bedrooms.to && property.propertyStatus[0] === propertyStatus)
          }
        }else{
      ...
      if (params.area) {
        if(propertyStatus){
          if (params.area.from) {
            data = data.filter(property => property.area.value === '' ? (property.area.value >= params.area.from && property.propertyStatus[0] === propertyStatus) : (property.area.value2 >= params.area.from && property.propertyStatus[0] === propertyStatus))
          }
          if (params.area.to) {
            data = data.filter(property => property.area.value === '' ? (property.area.value >= params.area.to && property.propertyStatus[0] === propertyStatus) : (property.area.value2 >= params.area.to && property.propertyStatus[0] === propertyStatus))
          }
        }else{
        if (params.area.from) {
          data = data.filter(property => property.area.value === '' ? property.area.value >= params.area.from : property.area.value2 >= params.area.from)
        }
        if (params.area.to) {
          data = data.filter(property => property.area.value === '' ? property.area.value >= params.area.to : property.area.value2 >= params.area.to)
        }
      }
      }
      if (params.yearBuilt) {
  ...
  public getTestimonials(): Observable<any> {
    return this.http.get(this.ourtestimonialListApi);
  }
  ...
  public getAgentsList() {
    return this.http.get(this.agentListApi);
    }
